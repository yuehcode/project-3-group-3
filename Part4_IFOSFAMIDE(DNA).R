...
library(DESeq2)
# load counts
cnts <- read.csv('dna_damage_saline_deseq.csv',row.names=1)

# filter out rows that have any zeros for funzies
cnts <- subset(cnts,rowSums(cnts==0)==0)

# sample information
info <- read.csv('group_3_saline_rna_info.csv')
...
write.csv(res,'IFOSFAMIDE_dna_damage_deseq_results.csv')

# write out matrix of normalized counts(we dont need this step)
# write.csv(counts(dds,normalized=TRUE),'dna_damage_saline_deseq_norm_counts.csv')

# Report the number of genes significant at p-adjust < 0.05
sum(res$padj < 0.05, na.rm=TRUE ) #127


# A table of the top 10 DE genes 
res<- read.csv(header = T,'IFOSFAMIDE_dna_damage_deseq_results.csv')
gene_sortbypvalue <- (res[order(res$pvalue), ])
names(gene_sortbypvalue)[1]<-"Geneid"
write.csv(gene_sortbypvalue[1:10,] , 'top_ten_IFOSFAMIDE.csv')

# histograms of fold change values from the significant DE genes
resSig <- res[ which(res$padj < 0.05 ), ]
hist( resSig$log2FoldChange, breaks=100, col="grey", main = "Foldchange of Ifosfamide group from the significant DE genes", xlab = "log2(foldchange)" )
...
volcano_IFO+ggtitle("Volcano plot for Ifosfamide group")
